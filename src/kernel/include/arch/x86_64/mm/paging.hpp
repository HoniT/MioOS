...
#ifndef PAGING_X86_64_HPP
#define PAGING_X86_64_HPP
...
namespace arch::mem {
    ...
    // Actual HAL implementation
    class X86_64PagingBackend final : public IPagingBackend {
    public:
        ...
        explicit X86_64PagingBackend(VirtAddr hhdm_offset = HHDM_BASE) noexcept;
        X86_64PagingBackend(PhysAddr boot_pml4_phys,
                        VirtAddr hhdm_offset) noexcept;

        ~X86_64PagingBackend() noexcept override = default;

        X86_64PagingBackend(const X86_64PagingBackend&)            = delete;
        X86_64PagingBackend& operator=(const X86_64PagingBackend&) = delete;

        // IPagingBackend Interface

        [[nodiscard]] PagingContext create_context()                          noexcept override;
        void                        destroy_context(PagingContext ctx)        noexcept override;

        [[nodiscard]] PagingError   map_page(PagingContext ctx, VirtAddr virt,
                                            PhysAddr phys, PageFlags flags)  noexcept override;
        [[nodiscard]] PagingError   unmap_page(PagingContext ctx, VirtAddr v)  noexcept override;
        [[nodiscard]] PagingError   protect_page(PagingContext ctx, VirtAddr v,
                                                PageFlags flags)              noexcept override;

        [[nodiscard]] PhysAddr      translate(PagingContext ctx,
                                            VirtAddr virt) const             noexcept override;

        void                        activate(PagingContext ctx)                noexcept override;
        void                        flush_tlb_page(VirtAddr virt)             noexcept override;
        void                        flush_tlb_full()                          noexcept override;

        void                        share_kernel_mappings(PagingContext ctx)  noexcept override;
        [[nodiscard]] PagingContext kernel_context() const                    noexcept override;
