...
//
// x86_64 paging bare metal logic
// ========================================
...
namespace mem {
    ...
    uint64_t PagingBackend::rdmsr(uint32_t msr) noexcept {
        uint32_t lo, hi;
        asm volatile("rdmsr" : "=a"(lo), "=d"(hi) : "c"(msr));
        return (static_cast<uint64_t>(hi) << 32) | lo;
    }

    void PagingBackend::wrmsr(uint32_t msr, uint64_t val) noexcept {
        asm volatile("wrmsr"
            : : "c"(msr),
    ...
    PagingBackend::PagingBackend(VirtAddr hhdm_offset) noexcept
    ...
    PagingBackend::PagingBackend(PhysAddr boot_pml4_phys,
    ...
    PhysAddr PagingBackend::alloc_page_table() noexcept {
        PhysAddr pa = reinterpret_cast<PhysAddr>(PMM::alloc_frame());
        if (!pa) return 0;
    ...
    PagingContext PagingBackend::create_context() noexcept {
    ...
    void PagingBackend::destroy_context(PagingContext ctx) noexcept {
    ...
    void PagingBackend::share_kernel_mappings(PagingContext ctx) noexcept {
    ...
    PagingContext PagingBackend::kernel_context() const noexcept {
    ...
    PagingError PagingBackend::unmap_page(
    ...
    PagingError PagingBackend::protect_page(
    ...
    PhysAddr PagingBackend::translate(
    ...
    void PagingBackend::activate(PagingContext ctx) noexcept {
        if (ctx.is_valid())
            write_cr3(ctx.value);
    }

    PagingContext PagingBackend::current_context() noexcept {
